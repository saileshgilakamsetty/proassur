...
cron.schedule('* 59 23 * * 6', async function () {
   /// console.log("\n\n---------------------------- Corn Job Start At " + moment().format('MMMM Do YYYY, hh:mm:ss') + " ----------------------------\n");
    ...
    try{
        ...
        for (let i = 0; i < getAllSessionData.length; i++) {
            ...
            if (diff < 0) {
                try{
                    let deleteSessionId = "DELETE from `im_usersessions` where `token`=?";
                    await mysql.execute(deleteSessionId,[getAllSessionData[i].token]);
...
async function clearDisconnectedOldSockets() {
    ...
    try{
        ...
        try{
            let updateActiveQuery = "UPDATE `users` SET `active` = ?";
    ...
    socket.on("addMember",async function (res) {
        ...
        if (typeof res === 'object') {
            ...
            if (await isValidToken(data._r)) {
                ...
                socket.disconnect();
                console.log("[ " + moment().format('MMMM Do YYYY, hh:mm:ss') + " ] " + "invalid user arvind");
            }
    ...
    socket.on("deleteMember",async function (res) {
        ...
        if (typeof res === 'object') {
            ...
            if (await isValidToken(data._r)) {
                ...
                socket.disconnect();
                console.log("[ " + moment().format('MMMM Do YYYY, hh:mm:ss') + " ] " + "invalid 123");
            }
...
async function activeUser(userId) {
    let updateActiveQuery = "UPDATE `users` SET `active` = 1 WHERE `users`.`userId` = ?";
                    ...
                    for (let i = 0; i < res.length; i++) {
                        ...
                        try {
                            ...
                            for (let i = 0; i < res2.length; i++) {
                                ...
                                try {
                                    users[res2[i].socketId].emit("updateStatusOnReconnect", data);
                                } catch (err) {
                                    console.log("[ " + moment().format('MMMM Do YYYY, hh:mm:ss') + " ] " + err);
                                }
                            }
...
async function deactivateUser(userId) {
    ...
    try{
        ...
        if (res1.length === 0) {
            let updateActiveQuery = "UPDATE `users` SET `active` = 0 WHERE `users`.`userId` = ?";
