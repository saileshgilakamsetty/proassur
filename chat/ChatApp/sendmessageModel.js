...
Im_message_Model.getRecentMessage = async (g_id)=>{
    ...
    if(result.length>0){
        ...
        if(prepareData.type!="text" && prepareData.type!="update" && prepareData.type!="document"){
            prepareData.message= serverConf.getHost()+"assets/im/group_"+g_id+"/"+prepareData.message;
        }
        ...
        if(prepareData.type=="video"){
            prepareData.poster=serverConf.getHost()+"assets/img/poster.jpg";
        }
        return prepareData;
    }
};
...
Im_message_Model.getRecentMessageWithUpdate = async (g_id)=>{
    ...
    if(result.length>0){
        ...
        if(prepareData.type=="document"){
            let fileUrl=encodeURIComponent(serverConf.getHost() + "assets/im/group_"+prepareData.receiver+"/"+prepareData.message)+"&fn="+encodeURIComponent(prepareData.fileName);
            prepareData.message=serverConf.getHost()+"download?f="+fileUrl;
        }
        if(prepareData.type=="video"){
            prepareData.poster=serverConf.getHost()+"assets/img/poster.jpg";
