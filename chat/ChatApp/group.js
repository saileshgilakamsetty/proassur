...
app.get_user = async (u_id) =>{
    let userData = await mysqlCon.execute("SELECT * FROM `users` WHERE `userId` = ?", [u_id]);
    let url = serverConf.getHost() + "assets/img/download.png";
    if (userData[0].userProfilePicture !== null) {
        url = serverConf.getHost() + "assets/userImage/" + userData[0].userProfilePicture;
    }
    return {
...
app.messageProcess=async (message) =>{
    ...
    if(message.type!="text" && message.type!="document" && message.type!="update"){
        message.message=serverConf.getHost()+ "assets/im/group_"+message.receiver+"/"+message.message;
    }
    if(message.type=="document"){
        let fileUrl=encodeURIComponent(serverConf.getHost() + "assets/im/group_"+message.receiver+"/"+message.message)+"&f="+encodeURIComponent(message.fileName);
        message.message=serverConf.getHost()+"download?u="+fileUrl;
    }
    if(message.type=="video"){
        message.poster=serverConf.getHost()+"assets/img/poster.jpg";
    }
...
app.get_group = async (g_id, u_id) => {
    ...
    if(membersInfo.length===0){
        groupImage.push(serverConf.getHost() + "assets/img/download.png");
