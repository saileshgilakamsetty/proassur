...
// function to send mail to registered email address to reset password
    ...
    function dashboard() {
        ...
        if ($data['user_role'] == 4) { // company
            ...
            // Pending Slips
            $data['todays_total_pending_slips'] = $this->front_model->getPendingSlips($status = 0);
    ...
    function policyFetchByDate() {
        ...
        if ($data['user_role'] == 4) { // company
            ...
            // echo $searchkey;
            $result['insurance_policies'] = $this->front_model->getPolicyListForCompanyByDate($company_id,$strId,$endId,$searchkey);
            $data = $result['insurance_policies'];
        ...
        // print_r($result['insurance_policies']);
       // return $result;
    }

    function policyFetchByPaymentStatus() {
        CheckUserLoginSession();
        $user_id = $this->session->userdata('user_id');
        $data['user_role'] = $this->session->userdata('role');
        $user_data = $this->session->userdata();
        if ($data['user_role'] == 4) { // company
            ...
            $pymnts_id = $this->input->post(pymnts_id);
            // echo $pymnts_id;
            if($pymnts_id ==""){
                $result['insurance_policies'] = $this->front_model->getPolicyListForCompany($company_id, 'all');
                print_r($result);die;
                $data = $result['insurance_policies'];
                print json_encode($data);die;
            }else{
                $result['insurance_policies'] = $this->front_model->getPolicyListForCompanyByPaymentStatus($company_id,$pymnts_id);
                //print json_encode($data);
                $data = $result['insurance_policies'];
                 print json_encode($data);
            }
