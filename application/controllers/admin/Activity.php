	...
	public function add() {
		...
		if(!empty($post_data)) {       
			...
			if($this->form_validation->run() == FALSE) {   } else {
				...
				$id              = $this->admin_model->setInsertData($this->activity,$data);

				if($_FILES["image"]["name"] != "") {
					$image             = do_upload('user','image');
					$data_featured_img = array('image' => $image );
					$this->admin_model->setUpdateData($this->activity,$data_featured_img,$id);
				}
				$this->session->set_flashdata('message','Your Activity has been added successfully');
		        redirect('admin/activity/lists','refresh');
		    }
        }
	...
	public function edit() {
		...
		if(!empty($post_data)) {        
			...
			if($this->form_validation->run() == FALSE) {   } else {
				...
				$id              = $this->admin_model->setUpdateData($this->activity,$data,$id);
				if($_FILES["image"]["name"] != "") {
				...
				$this->session->set_flashdata('message','Your activity has been update successfully');
		        redirect('admin/activity/lists','refresh');
		    }
        }
		$data['dataCollection']               = $this->admin_model->getDataCollectionByID($this->activity,$id);
		$this->load->view('admin/include/head');
		$this->load->view('admin/include/header');
		$this->load->view('admin/include/sidebar');
...
// function to list all activity
	...
	public function lists()	{
        ...
        $limit                  = $per_page;
        $totalCount             = $this->admin_model->totalRecord($this->activity);
		$data["dataCollection"] = $this->admin_model->getDataCollection($this->activity,$limit,$start);
...
// function to delete a activity
	...
	public function delete() {
		CheckAdminLoginSession();
		$id=$this->uri->segment(4);
		$this->admin_model->dataDelete($this->activity,$id);
		$this->session->set_flashdata('message','Your activity has been deleted successfully');
        redirect('admin/activity/lists','refresh');
	}

// function to change status of activity
	...
	public function status() {
		...
		CheckAdminLoginSession();		
		$data['status'] = $status;				        	             		 
		$this->admin_model->setUpdateData($this->activity,$data,$id);
