<div class="content-wrapper">
  ...
  <section class="content-header">
    ...
    <div class="header-title">    
      ...
      <small><?=getContentLanguageSelected('WARRANTY_LIST',defaultSelectedLanguage())?></small>
      ...
      <div class="col-sm-12">
        ...
        <div class="panel panel-bd">
          <div class="panel-heading">
            ...
            <div class="btn-group"> 
              <a class="btn btn-success" href="<?=base_url('admin/warranty/add')?>"> <i class="fa fa-plus"></i>  <?=getContentLanguageSelected('ADD_WARRANTY',defaultSelectedLanguage())?></a>  
            </div>
              ...
              <table id="example2" class="table table-bordered table-hover">
                <thead>
                  <tr> 
                    ...
                    <th><?=getContentLanguageSelected('BRANCH_NAME',defaultSelectedLanguage())?>
                    <th><?=getContentLanguageSelected('RISK_NAME',defaultSelectedLanguage())?></th>
                ...
                <tbody>
                  ...
                  <?php if(!empty($dataCollection)){  foreach ($dataCollection as $data) { ?>
                            
                    <tr>         
                      <td><?=getWarrantyName($data->warranty_name_id)?></td>
                      <td><?=getCompanyName($data->company_id)?></td>
                      <td><?=getBranchName($data->branch_id)?></td>
                      <td><?=getRisqueName($data->risque_id)?></td>

                      <?php if($data->status==1){ ?>
                        <td width="70"><a title="Click to Inactive" href="<?=base_url('admin/warranty/status/'.$data->id.'/0')?>"  data-toggle="tooltip" data-placement="left" class="label-default label label-success">Active</a></td>
                      <?php } else { ?>
                        <td width="70"><a title="Click to Active" href="<?=base_url('admin/warranty/status/'.$data->id.'/1')?>"  data-toggle="tooltip" data-placement="left" class="label-default label label-danger">Inactive</a></td>
                      <?php } ?>
                      <td width="10%" >
                        <a href="<?=base_url('admin/warranty/edit/'.$data->id)?>" class="label-default label label-success" data-toggle="tooltip" data-placement="left" title="Edit"><i class="fa fa-pencil" aria-hidden="true"></i></a>
                        <a  onclick="return confirm('Sure you want to delete')" href="<?=base_url('admin/warranty/delete/'.$data->id)?>" class="label-default label label-danger" data-toggle="tooltip" data-placement="right" title="Delete "><i class="fa fa-trash-o" aria-hidden="true"></i></a>
                      </td>
                    </tr>
                  <?php }} ?>           
                </tbody>
