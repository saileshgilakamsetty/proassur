        ...
        <div class="row">
            ...
            <?php } ?>
                <div class="panel panel-bd">
                    <div class="panel-heading">
                        ...
                        <div class="btn-group"> 
                            <a class="btn btn-success" href="<?=base_url('admin/health-insurance-quote/add')?>"> <i class="fa fa-plus"></i>  <?=getContentLanguageSelected('ADD_QUOTE',defaultSelectedLanguage())?></a>  
                        </div>
                    </div>
            <div class="panel-body">
              ...
              <tr >
                        ...
                        <td><?=$data->min_days?></td> 
                        <td><?=$data->max_days?></td> 
                        <td><?= getPolicyCoverageArea($data->area)?></td> 
                        <!-- <td><?=$data->child_below_age?></td> 
                        <td><?=$data->adult_above_age?></td> 
                        <td><?=$data->surprime_above_age?></td>  -->

                       <?php if($data->status==1){ ?>
                       <td width="70"><a title="Click to Inactive" href="<?=base_url('admin/health-insurance-quote/status/'.$data->id.'/0')?>"  data-toggle="tooltip" data-placement="left" class="label-default label label-success">active</a></td>
                       <?php } else { ?>
                       <td width="70"><a title="Click to Active" href="<?=base_url('admin/health-insurance-quote/status/'.$data->id.'/1')?>"  data-toggle="tooltip" data-placement="left" class="label-default label label-danger">Inactive</a></td>
                       <?php } ?>
                       <td width="10%" >
                        <a href="<?=base_url('admin/health-insurance-quote/edit/'.$data->id)?>" class="label-default label label-success" data-toggle="tooltip" data-placement="left" title="Edit"><i class="fa fa-pencil" aria-hidden="true"></i></a>
                        <a  onclick="return confirm('Sure you want to delete')" href="<?=base_url('admin/health-insurance-quote/delete/'.$data->id)?>" class="label-default label label-danger" data-toggle="tooltip" data-placement="right" title="Delete "><i class="fa fa-trash-o" aria-hidden="true"></i></a>
                    </td>
                </tr>
                <?php }} ?>
