...
if (!function_exists('do_upload')) {
    ...
    function do_upload($folder = "", $filename = "") {
        ...
        $config['allowed_types'] = 'gif|jpg|png|jpeg|xls|xlsx|csv|pdf|bmp';
...
// function added by Shiv to get Payment Status options
...
if (!function_exists('getCompanyInsuredFor')) {
    ...
    function getCompanyInsuredFor($insured_id, $insurance_type_id, $table) {
        ...
        if ($insurance_type_id == 1) {// vehicle
            ...
            CI()->db->where('individual_insurance_option_details_id', getIndividualAccidentOptionDetailsId($insured_id));
...
if(!function_exists('getPaymentDataForReport')) {
    function getPaymentDataForReport($insurance_type_id,$insured_id,$quittance_id = null) {
        CI()->db->where("insurance_type_id",$insurance_type_id);
        CI()->db->where("insured_id",$insured_id);
        CI()->db->join("tbl_quittance",'tbl_payment.policy_number = tbl_quittance.policy_number');
        CI()->db->where('tbl_quittance.id',$quittance_id);
        $query = CI()->db->get('tbl_payment');
        if($query->num_rows() > 0) {
            $payment_data = $query->result();
            foreach(array_reverse($payment_data) as $key => $value) {
                ...
                $result['payment_details']['Numéro de police'] = $value->policy_number; 
                $result['payment_details']["Date d'effet"] 	   = date('Y-m-d',strtotime($value->policy_start_date));
                $result['payment_details']["Date d'échéance"]  = date('Y-m-d',strtotime($value->policy_end_date));
                $result['payment_details']["Date de création"] = date('Y-m-d',strtotime($value->policy_creation_date));
                $result['payment_details']["Type d 'assurance Nom"] = getInsuranceTypeName($value->insurance_type_id);
                $result['payment_details']['Nom']              = getUserName($value->user_id); 
                $result['payment_details']['Adresse']          = getUserDetailByUserId($value->user_id)['address']; 
                $result['payment_details']['Ville']            = getUserDetailByUserId($value->user_id)['city']; 
                // $result['payment_details']['Country']       = getUserDetailByUserId($value->user_id)['country']; 
                // $result['payment_details']['Fixed Phone']   = getUserDetailByUserId($value->user_id)['mobile']; 
                ...
                $result['payment_details']['Statut du paiement'] = $payment_status; 
                //$result['payment_details']['amount']          = $value->amount; 
                // Vehicle Insurance Data
                if($value->insurance_type_id == 1) {
                    // data of finalize company
                    ...
                    if($query1->num_rows() > 0) {
                        foreach ($query1->result() as $data_final) {
                            if($data_final->type == 'warranties') {
                                ...
                                $result['data_final']["Compagnie d'assurance"] = $data_final->company_name;
                                if($data_final->name == 'net_premium') {
									$days 			   = getDaysFromVehicleDetailId($value->insured_id,'tbl_selected_premium');
									$premium_rate      = getPremiumRateViaCompanyDays($days,getCompanyIdByVehicleDetailId($value->insured_id));
									$estimation_amount = getEstimationAmountByInsurerIdInsuranceType($value->insured_id, $value->insurance_type_id);
									//$amt =($premium_rate*$estimation_amount)/100;
                                    $amt = $value->amount;
                                    $label_vehicle = "Prime nette";
                                } else if($data_final->name == 'accessories') {
									//$amt = $data_final->value;
									$amt = $value->accessories;
                                    $label_vehicle = "Accessoires";
                                } else if($data_final->name == 'tax') {
									//$amt = $data_final->value;
									$amt = $value->tax;
                                    $label_vehicle = 'Taxes';
                                } else if($data_final->name == 'total_premium') {
									//$amt = $data_final->value;
									$amt = $value->total_amount;
                                    $label_vehicle = 'Prime totale';
                                } else if($data_final->name == 'required') {
									//$amt = $data_final->value;
                                    $label_vehicle = 'required';
                                }
                                $result['data_final'][$label_vehicle] = $amt;
                    ...
                    if($query2->num_rows() > 0) {
                        foreach ($query2->result() as $key => $vehicle_detail) {
                            $vehicle_basic_info = getVehicleBasicInfo($vehicle_detail->vehicle_basic_info_id);
                            ...
                            $result['vehicle_detail']['Marque'] = ucfirst(getName($vehicle_basic_info->make_id,'tbl_vehicle_make'));
                            $result['vehicle_detail']['Tarif appliqué']    = $vehicle_detail->tariff_code;
                            $result['vehicle_detail']['Designation']       = $vehicle_basic_info->designation;
                            $result['vehicle_detail']['Energie']           = $vehicle_basic_info->horse_power;
                            $result['vehicle_detail']['Puissance fiscale'] = $vehicle_basic_info->fiscal_power;
                            $result['vehicle_detail']['Poids autorisé']    = $vehicle_detail->authroise_weight;
                            $result['vehicle_detail']['Valeur catalogue']  = $vehicle_detail->catalogue_value;
                            $result['vehicle_detail']['Date de première immatriculation'] = ($vehicle_detail->date_first_release != '')?date('Y-m-d',strtotime($vehicle_detail->date_first_release)):'Not Available';
                            $result['vehicle_detail']['Immatriculation']   = $vehicle_detail->immatriclulation;
                            $result['vehicle_detail']['Usage']             = getName($vehicle_detail->usage,'tbl_usage');
                        }
                    }
                    ...
                    if($query4->num_rows() > 0) {
                        foreach ($query4->result() as $vehicle_secondry_driver) {
                            $result['vehicle_secondry_driver']['owner']               = $vehicle_secondry_driver->owner;
                            $result['vehicle_secondry_driver']['name']                = $vehicle_secondry_driver->name;
                            $result['vehicle_secondry_driver']['issue_date_license']  = date('Y-m-d',strtotime($vehicle_secondry_driver->issue_date_license));
                            $result['vehicle_secondry_driver']['year_license_expire'] = $vehicle_secondry_driver->year_license_expire;
                            $result['vehicle_secondry_driver']['license_number']      = $vehicle_secondry_driver->license_number;
                            $result['vehicle_secondry_driver']['permit_id']           = $vehicle_secondry_driver->permit_id;
                            $result['vehicle_secondry_driver']['double_command']      = $vehicle_secondry_driver->double_command;   
                    ...
                    $selected_transported_person_data = getLatestTransPersonInsuranceData('tbl_selected_vehicle_trans_insurance',array('vehicle_detail_id' => $value->insured_id));
                    CI()->db->where('id',$selected_transported_person_data->vehicle_trans_person_insurance_id);
                    $query7 = CI()->db->get('tbl_vehicle_trans_person_insurance');
                    if($query7->num_rows() > 0) {
                        $trans_person_info      = $query7->row();
                        $result['trans_person_insurance']['Libellé']         = $trans_person_info->title;
                        $result['trans_person_insurance']['Invalidité']         = $trans_person_info->death;
                        $result['trans_person_insurance']['Décès']    = $trans_person_info->disability;
                        $result['trans_person_insurance']['Frais Medicaux']  = $trans_person_info->medical_fees;
                    }
                ...
                // Health Insurance Data 
                    ...
                    if($query1->num_rows() > 0) {
                        foreach ($query1->result() as $data_final_health) {
                            $result['data_final_health']["Compagnie d'assurance"] = getCompanyName($data_final_health->company_id);

                            if($data_final_health->name == 'net_premium') {
                                $label_health = "Prime nette";
                                $amt = $value->amount;
                            } else if($data_final_health->name == 'accessories') {
                                $label_health = "Accessoires";
                                $amt = $value->accessories;
                            } else if($data_final_health->name == 'tax') {
                                $label_health = 'Taxes';
                                $amt = $value->tax;
                            } else if($data_final_health->name == 'total_premium') {
                                $label_health = 'Prime totale';
                                $amt = $value->total_amount;
                            } else {
                                $label_health = 'required';
                                $amt = $data_final_health->amount;
                            }

                            $result['data_final_health'][$label_health] = $amt;
                    ...
                    if($query2->num_rows() > 0) {
                        foreach ($query2->result() as $health_insurance_detail) {
                            // $result['health_insurance_detail']['user_id'] = $health_insurance_detail->user_id;
                            $result['health_insurance_detail']["Type d'assurance"] = getName($health_insurance_detail->health_insurance_type_id,'tbl_health_insurance_type');
                            if($health_insurance_detail->health_insurance_type_id == 1) {
                                $result['health_insurance_detail']['Nom du chef de famille'] = $health_insurance_detail->name_of_chief;
                                $result['health_insurance_detail']['Age'] = $health_insurance_detail->age_of_chief;
                                $result['health_insurance_detail']["Nombre d'assurés"] = $health_insurance_detail->persons_insured;
                            } else {
                            ...
                            $result['health_insurance_detail']["Date d'effet de la couverture"] = date('Y-m-d',strtotime($health_insurance_detail->start_date));
                            $result['health_insurance_detail']["Date de fin de la couverture"]  = date('Y-m-d',strtotime($health_insurance_detail->end_date));
                            $result['health_insurance_detail']["Zone de couverture de la police"] = getName($health_insurance_detail->policy_coverage_area_id,'tbl_policycoverage_area');
                            $result['health_insurance_detail']["Taux de remboursement"] = $health_insurance_detail->claim_reimbursement_rate;
                            $result['health_insurance_detail']['Montant à payer'] = $health_insurance_detail->amount_to_pay;
                            if($health_insurance_detail->health_insurance_type_id == 1) {
                                CI()->db->where('persons_insured_id',$value->insured_id);
                                $query3 = CI()->db->get('tbl_health_insurance_person_details');
                                if($query3->num_rows() > 0) {
                ...
                // Travel Insurance Data
                    ...
                    if($query1->num_rows() > 0) {
                        foreach ($query1->result() as $data_final_travel) {
                            $result['data_final_travel']["Compagnie d'assurance"] = getCompanyName($data_final_travel->company_id);
                            if($data_final_travel->name == 'net_premium') {
                                $label_travel = "Prime nette";
                                $amt = $value->amount;
                            } else if($data_final_travel->name == 'accessories') {
                                $label_travel = "Accessoires";
                                $amt = $value->accessories;
                            } else if($data_final_travel->name == 'tax') {
                                $amt = $value->tax;
                                $label_travel = 'Taxes';
                            } else if($data_final_travel->name == 'total_premium') {
                                $label_travel = 'Prime totale';
                                $amt = $value->total_amount;
                            } else {
                                $label_travel = 'Cout de police';
                                $amt = $data_final_travel->amount;
                            }
                            $result['data_final_travel'][$label_travel] = $amt;
                    ...
                    if($query2->num_rows() > 0) {
                        foreach ($query2->result() as $travel_people_insured) {
                            // $result['travel_people_insured']['user_id'] = $travel_people_insured->user_id;
                            $result['travel_people_insured']["Nombre d'assurés"] = $travel_people_insured->people_insured;
                        }
                    }
                    ...
                    if($query3->num_rows() > 0) {
                        $l = 1; 
                        foreach ($query3->result() as $travel_people_details) {
                            $result['travel_people_details']["Prenom de l'assuré " .$l] = $travel_people_details->first_name. " ".$travel_people_details->last_name;
                            ...
                            $result['travel_people_details']["Age de l'assuré ".$l]         = $travel_people_details->age;
                            $l++;
                        }
                    }
                    ...
                    if($query4->num_rows() > 0) {
                        foreach ($query4->result() as $travel_destination_details) {
                            $result['travel_destination_details']['Date de départ'] 		   = date('Y-m-d',strtotime($travel_destination_details->travel_start_date));
                            $result['travel_destination_details']['Date de retour'] 		   = date('Y-m-d',strtotime($travel_destination_details->travel_end_date));
                            $result['travel_destination_details']['Destination']    		   = $travel_destination_details->destination_of_trip;
                            $result['travel_destination_details']['Nombre total de voyageurs'] = $travel_destination_details->total_travelers;
                        }
                    }
                }
                ...
                // Professional Multirisk Insurance Data
                    ...
                    if($query1->num_rows() > 0) {
                        foreach ($query1->result() as $data_final_professional_multirisk) {
                            ...
                            if($data_final_professional_multirisk->name == 'net_premium') {
                                $label_profrssional_multirisk = "Prime nette";
                                $amt = $value->amount;
                            } else if($data_final_professional_multirisk->name == 'accessories') {
                                $label_profrssional_multirisk = "Accessoires";
                                $amt = $value->accessories;
                            } else if($data_final_professional_multirisk->name == 'tax') {
                                $amt = $value->tax;
                                $label_profrssional_multirisk = 'Taxes';
                            } else if($data_final_professional_multirisk->name == 'total_premium') {
                                $amt = $value->total_amount;
                                $label_profrssional_multirisk = 'Prime totale';
                            } else {
                                $label_profrssional_multirisk = 'Cout de police';
                                $amt = $data_final_professional_multirisk->value;
                            }


                            $result['data_final_professional_multirisk'][$label_profrssional_multirisk] = $data_final_professional_multirisk->value;
                    ...
                    if($query2->num_rows() > 0) {
                        foreach ($query2->result() as $proffesional_multirisk_quote_details) {
                            ...
                            $result['proffesional_multirisk_quote_details']['Adresse'] = $proffesional_multirisk_quote_details->address;
                            $result['proffesional_multirisk_quote_details']['Adresse professionnelle'] = $proffesional_multirisk_quote_details->business_address;
                            $result['proffesional_multirisk_quote_details']['Code pays'] = $proffesional_multirisk_quote_details->dial_code;
                            $result['proffesional_multirisk_quote_details']['Numéro de téléphone'] = $proffesional_multirisk_quote_details->contact_number;
                            $result['proffesional_multirisk_quote_details']['Document'] = $document;
                            $result['proffesional_multirisk_quote_details']['Police tacite'] = $tacit_policy;
                            $result['proffesional_multirisk_quote_details']["Nom de l'activité choisie"] = getName($proffesional_multirisk_quote_details->activity_id,'tbl_activity');
                            $result['proffesional_multirisk_quote_details']['Capital assuré'] = $proffesional_multirisk_quote_details->capital_insured;
                            $result['proffesional_multirisk_quote_details']["Compagnie d'assurance"] = getCompanyName($proffesional_multirisk_quote_details->company_selected);
                            $result['proffesional_multirisk_quote_details']['Branche'] = getBranchName(getProffesionalBranchId());
                            $result['proffesional_multirisk_quote_details']['Risque'] = getRisqueName(getProffesionalRisqueId());
                        }
                    }
                }
                ...
                // Individual Accident Insurance Data
                    ...
                    if($query1->num_rows() > 0) {
                        ...
                        if($query2->num_rows() > 0) {
                            foreach ($query2->result() as $data_final_individual_accident) {
                                $result['data_final_individual_accident']["Compagnie d'assurance"] = getCompanyName($data_final_individual_accident->company_id);

                                if($data_final_individual_accident->title == 'net_premium') {
                                    $label_individual_accident = "Prime nette";
                                    $amt = $value->amount;
                                } else if($data_final_individual_accident->title == 'accessories') {
                                    $label_individual_accident = "Accessoires";
                                    $amt = $value->accessories;
                                } else if($data_final_individual_accident->title == 'tax') {
                                    $label_individual_accident = 'Taxes';
                                    $amt = $value->tax;
                                } else if($data_final_individual_accident->title == 'total_premium') {
                                    $label_individual_accident = 'Prime totale';
                                    $amt = $value->total_amount;
                                } else {
                                    $label_individual_accident = 'Cout de police';
                                    $amt = $data_final_individual_accident->amount_to_pay;
                                }


                                $result['data_final_individual_accident'][$label_individual_accident] = $amt;
                    ...
                    if($query2->num_rows() > 0) {
                        foreach ($query3->result() as $individual_accident_person_details) {
                            ...
                            // $result['individual_accident_person_details']['user_id'] = $individual_accident_person_details->user_id;
                            $result['individual_accident_person_details']["Nom de l'activité"] = getName($individual_accident_person_details->individual_accident_activity_id,'tbl_activity');
                            $result['individual_accident_person_details']['Nom'] = $individual_accident_person_details->name;
                            $result['individual_accident_person_details']['Adresse'] = $individual_accident_person_details->address;
                            $result['individual_accident_person_details']['Adresse professionnelle'] = $individual_accident_person_details->business_address;
                            $result['individual_accident_person_details']['Code pays'] = $individual_accident_person_details->dial_code;
                            $result['individual_accident_person_details']['Numéro de téléphone'] = $individual_accident_person_details->contact_number;
                            // $result['individual_accident_person_details']['Document'] = $document;
                            $result['individual_accident_person_details']['Police à tacite reconduction'] = $tacit_policy;
                        }
                    }
                }
                ...
                // Credit Insurance Data
                    ...
                    if($query1->num_rows() > 0) {
                        foreach ($query1->result() as $data_final_credit) {
                            if($data_final_credit->type == 'warranties') {
                                ...
                                if($data_final_credit->name == 'net_premium') {
                                    $label_credit = "Prime nette";
                                    $amt = $value->amount;
                                } else if($data_final_credit->name == 'accessories') {
                                    $label_credit = "Accessoires";
                                    $amt = $value->accessories;
                                } else if($data_final_credit->name == 'tax') {
                                    $label_credit = 'Taxes';
                                    $amt = $value->tax;
                                } else if($data_final_credit->name == 'total_premium') {
                                    $label_credit = 'Prime totale';
                                    $amt = $value->total_amount;
                                } else {
                                    $label_credit = 'Cout de police';
                                    $amt = $data_final_credit->value;
                                }

                                $result['data_final_credit'][$label_credit] = $amt;
                    ...
                    if($query2->num_rows() > 0) {
                        foreach ($query2->result() as $credit_detail) {
                            // $result['credit_detail']['user_id']   = $credit_detail->user_id;
                            $result['credit_detail']['Insurance Start Date']     = date('Y-m-d',strtotime($credit_detail->credit_insurance_start_date));
                            $result['credit_detail']['Insurance Expiry Date']    = date('Y-m-d',strtotime($credit_detail->credit_insurance_expiry_date));
                            ...
                            // $result['credit_detail']['credit_tarification_id']           = $credit_detail->credit_tarification_id;
                            $result['credit_detail']["Compagnie d'assurance"] = getCompanyName($credit_detail->company_selected);
                        }
                    }
                ...
                // House Insurance Data
                    ...
                    if($query1->num_rows() > 0) {
                        foreach ($query1->result() as $data_final_house) {
                            if($data_final_house->type == 'warranties') {
                                ...
                                if($data_final_house->name == 'net_premium') {
                                    $label_house = "Prime nette";
                                    $amt = $value->amount;
                                } else if($data_final_house->name == 'accessories') {
                                    $label_house = "Accessoires";
                                    $amt = $value->accessories;
                                } else if($data_final_house->name == 'tax') {
                                    $label_house = 'Taxes';
                                    $amt = $value->tax;
                                } else if($data_final_house->name == 'total_premium') {
                                    $label_house = 'Prime totale';
                                    $amt = $value->total_amount;
                                } else {
                                    $label_house = 'Cout de police';
									$amt = $data_final_house->value;
                                }

                                $result['data_final_house'][$label_house] = $amt;
                    ...
                    if($query2->num_rows() > 0) {
                        foreach ($query2->result() as $house_detail) {
                            // $result['house_detail']['user_id'] = $house_detail->user_id;
                            $result['house_detail']["Qualité de l'assuré"] = getName($house_detail->insurer_quality_id,'tbl_insurer_quality');
                            $result['house_detail']['Nombre de chambres']  = $house_detail->room;
                            $result['house_detail']['Loyer mensuel'] = $house_detail->monthly_rent;
                            $result['house_detail']['Valeur du contenu'] = $house_detail->content_value;
                            $result['house_detail']["Valeur de l'immeuble"] = $house_detail->building_value;
                            $result['house_detail']['Superficie'] = $house_detail->superficy;
                            $result['house_detail']['Type de maison'] = getName($house_detail->house_type_id,'tbl_house_type');
                            $result['house_detail']['Catégorie'] = getName($house_detail->house_category_id,'tbl_house_category');
                            $result['house_detail']['Période assurée'] = getName($house_detail->month_id,'tbl_house_month');
                            $result['house_detail']['Du']  = $house_detail->from;
                            $result['house_detail']['Au']  = $house_detail->to;
                            $result['house_detail']['Nom du risque'] = getRisqueName($house_detail->risque_id);
                            $result['house_detail']["Compagnie d'assurance"] = getCompanyName($house_detail->company_selected);
                            // $result['house_detail']['house_tarification_id']          = $house_detail->house_tarification_id;
                        }
                    }
                }
        ...
        if ($insurance_type_id == 1) { // VEHICLE
            ...
            if ($query->num_rows() > 0) {
                ...
                foreach ($res as $key => $value) {
                    ...
                    $result['policy_start_date'] = date("Y-m-d H:i:s", strtotime($insurance_dates->start_date));
...
// function to get quittance number
if (!function_exists('getQuittanceNumberOptions')) {

    function getQuittanceNumberOptions($type) {
            $query  = CI()->db->get('tbl_quittance');
            $skills = $query->result();
            foreach ($skills as $value) {
                $options[''] = $type;
...
// function to get quittance number
if (!function_exists('getUserIdFromMobile')) {
