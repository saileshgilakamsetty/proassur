...
/*if(!function_exists('getTaxAmount')){
	function getTaxAmount($net_premium_total_amount){
		// print_r($net_premium_total_amount);
		CI()->db->where('name','tax_percent');
		$query = CI()->db->get('tbl_settings');
		$count = $query->num_rows();
		if($count>0) {
			$res = $query->row();
			$result = ($res->value * $net_premium_total_amount)/100;
		}
		else {
			$result = 0;
		}
...
if(!function_exists('getCreditStartEndDate')){
	function getCreditStartEndDate($credit_id){
	    ...
	    CI()->db->select('credit_insurance_end_date');
...
if(!function_exists('getNetPremiumAccessoriesAndTax')) {
	function getNetPremiumAccessoriesAndTax($insurance_type_id,$insured_id) {
		...
		if($insurance_type_id == 5) { //INDIVIDUAL ACCIDENT
			...
			$individual_insurance_option_details_id =  $insured_id;
	/*		print_r($individual_insurance_option_details_id);
			echo "<br>";
			print_r(getIndividualInsuranceOptionDetailId($insured_id));
			die();*/
	    ...
	    if($id>0){
	        ...
	        if($count>0){        	
				...
				$result='<img src="'.base_url($result).'" class="">';					
		    	 ...
		    	 $result='<img src="'.base_url('assets/front/images/image-coming-soon.jpg').'" class="">';
		    }
	    }
	    return $result;
	}
...
/*if(!function_exists('getUserPolicyRecord')) {
	function getUserPolicyRecord($table,$data) {
		...
		if($query->num_rows() > 0) {
			...
			foreach ($res as $key => $value) {
				...
				$result[$key]['company_id']    = $value['company_id'];
...
// function added by Shiv to view the policies generated by Partner 
/*if(!function_exists('getPartnerPolicyRecord')) {
	function getPartnerPolicyRecord($data) {
		// CI()->db->where('user_id',$data['user_id']);
		CI()->db->select('*');
		CI()->db->from('tbl_quittance');
		CI()->db->join('tbl_payment','tbl_quittance.policy_number = tbl_payment.policy_number');
		CI()->db->where('tbl_payment.policy_created_by',3);
		$query = CI()->db->get();
		if($query->num_rows() > 0) {
			$res = $query->result_array();
			print_r($res);die;	
